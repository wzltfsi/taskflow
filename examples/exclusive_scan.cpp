...
int main(int argc, char* argv[]) {
  ...
  // sequential exclusive scan
  {
    ...
    std::exclusive_scan( elements.begin(), elements.end(), scan_seq.begin(), -1, std::plus<int>{});
  ...
  // create a parallel exclusive scan task
  {
    ...
    auto beg = std::chrono::steady_clock::now();
    taskflow.exclusive_scan( elements.begin(), elements.end(), scan_par.begin(), -1, std::plus<int>{});
