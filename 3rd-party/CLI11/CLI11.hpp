...
/// 在所有新行的开头添加一个领导者（在第一行的开头没有添加 /// ）。 `"; "` 用于 ini 文件
///
/// 不能使用正则表达式，否则这将是一个 subs.
...
/// 此函数检测参数后跟有转义引号的等号或冒号，然后修改字符串以用空格替换等号。 这是允许拆分函数正常工作所必需的，并且旨在与 find_and_modify 函数一起使用，返回值是 find_and_modify 函数所需的 offset+1。
...
struct is_vector<std::vector<T, A>> { static bool const value = true; };
...
template <> 
struct is_bool<bool> { static bool const value = true; };
...
// 如果不是短选项，则返回 false。 否则，设置 opt name 和 rest 并返回 true
...
// 如果不是长选项，则返回 false。 否则，设置 opt name 和 = 的另一边并返回 true
...
///
/// 这些是有用的简单 `std::string(const std::string&)` 验证器。 如果验证失败，它们会返回一个字符串。 还提供了自定义结构，具有相同的用户语义，但能够提供新的类型名称。
 
struct Validator {
   /// 这是类型名，如果为空则类型名不会改变
    std::string tname;

     /// 这是要调用的基本函数。 如果验证失败则返回字符串错误消息。
    std::function<std::string(const std::string &)> func;

    /// 这是验证器所需的运算符 - 提供帮助用户（CLI11 直接使用成员 `func`）
...
// 内置验证器的实现是使用 Validator 类； 用户只能使用 const（静态）版本（因为没有设置）。 因此，这是详细的。
namespace detail {

/// 检查现有文件（如果检查失败则返回错误消息）
...
// 这里不需要static，因为global const 隐含static。

/// 检查现有文件（如果检查失败则返回错误信息）
...
struct Range : public Validator {
    ///  这会产生一个包含最小值和最大值的范围。 请注意，构造函数是模板化的，但结构不是，因此不需要 C++17 来为 Range(a,b) 提供良好的语法。
    template <typename T> 
    Range(T min, T max) {
...
namespace detail {
/// 将字符串拆分为程序名和命令行参数 假定字符串包含文件名，后跟其他参数 返回值包含一对，第一个参数包含程序名，第二个参数包含其他所有内容
inline std::pair<std::string, std::string> split_program_name(std::string commandline) {
    ...
    while(!ExistingFile(commandline.substr(0, esp)).empty()) {
        ...
        if(esp == std::string::npos) {
            // 如果我们已经到了最后还没有找到一个有效的文件就假设第一个参数是程序名
...
/// 这是运行格式化程序的最低要求。 如果用户根本不关心其中的结构，则可以将其子类化
class FormatterBase {
  protected:
    /// @name Options
    ///@{
    ...
    /// @brief  所需的帮助打印输出标签（用户可更改）值为 Needs、Excludes 等。
...
class FormatterLambda final : public FormatterBase {
  ...
  public:
    /// 创建一个带有 lambda 函数的 FormatterLambda
...
/// 这是 CLI11 的默认格式化程序。 它漂亮地打印帮助输出，并被分解成许多可重写的方法，以最小的努力实现高度可定制。
class Formatter : public FormatterBase {
  public:
    ...
    /// @brief 这是选项的名称部分，默认：左列
    virtual std::string make_option_name(const Option *, bool) const;

    /// @brief 这是名称的选项部分，默认：合并到左列
    virtual std::string make_option_opts(const Option *) const;

    /// @brief 这是描述。 默认值：右列，如果左列太大则换行
    virtual std::string make_option_desc(const Option *) const;

    /// @brief 这用于在 USAGE 行上打印名称
...
// 这是 Option 和 OptionDefaults 的 CRTP 基类。 以这种方式设计是为了共享部分课程； OptionDefaults 可以复制到 Option。
template <typename CRTP> class OptionBase {
  ...
  protected:
    ...
    /// `expected_ == 1` 时多个参数的策略（也可以在 bool 标志上设置）
    MultiOptionPolicy multi_option_policy_{MultiOptionPolicy::Throw};

    /// 将内容复制到另一个类似的类（一个基于OptionBase）
    template <typename T> 
